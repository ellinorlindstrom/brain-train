...
export default function History() {
  ...
  const history = useMemo(() => getTestHistory(selected), [selected])
  const domain = getDomain(selected)

  const chartData = history.map((h) => ({
    label: new Date(h.date).toLocaleDateString('sv-SE', { day: 'numeric', month: 'short' }),
    score: h.score,
  }))
    ...
    <div className="max-w-3xl mx-auto py-10 px-4 flex flex-col gap-6">
      ...
      <div className="bg-slate-900 rounded-2xl p-6">
        <h2 className="font-semibold mb-4" style={{ color: domain.color }}>
          {domain.name} — poäng per test
        </h2>
        {chartData.length === 0 ? (
          <p className="text-slate-500 text-sm py-10 text-center">
            Inga test genomförda ännu för det här området.
          </p>
        ) : (
      ...
      {history.length > 0 && (
        <div className="bg-slate-900 rounded-2xl p-6">
          <h2 className="text-sm text-slate-500 uppercase tracking-wide mb-3">Alla test</h2>
            ...
            {[...history].reverse().map((h) => (
              ...
              >
                <span className="text-slate-400">
                  {new Date(h.date).toLocaleString('sv-SE', {
                    day: 'numeric',
                    month: 'short',
                    hour: '2-digit',
                    minute: '2-digit',
